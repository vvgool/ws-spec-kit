...
export async function runTrustedGate(input: { cwd: string; workItemId: string; stageId: string; attemptId: string; gateId: string }): Promise<TrustedEvidence> {
  ...
  if (gate === undefined) throw new VerificationError("WSPEC_GATE_NOT_FOUND", `找不到 Gate ${input.gateId}。`);
  ...
  const content = canonicalize(unsigned); if (content === undefined) throw new VerificationError("WSPEC_EVIDENCE_INVALID", "Evidence 无法规范化。");
  ...
  return mutateControlPlane({
    ...
    mutate: async (current) => {
      if (await computeWorkspaceTreeDigest(data.worktree) !== evidence.workspaceTreeDigest) throw new VerificationError("WSPEC_WORKSPACE_CHANGED", "Gate 执行后工作区发生变化，Evidence 未记录。");
...
export async function verifyWorkItem(input: { cwd: string; workItemId: string }): Promise<RuntimeProjection> {
  ...
  if (data.projection.workItem.status === "verified") {
    ...
    throw new VerificationError("WSPEC_WORKSPACE_CHANGED", "已验证工作区发生变化，旧 Evidence 已失效。");
  }
  ...
  for (const [gateId, gate] of gates) {
    ...
    if (gate.required && evidence.result !== "passed") { await transitionRuntime({ cwd: input.cwd, workItemId: input.workItemId, scope: "work-item", to: "blocked", idempotencyKey: `gate-failed:${evidence.evidenceId}` }); throw new VerificationError("WSPEC_REQUIRED_GATE_FAILED", `必需 Gate ${gateId} 未通过。`); }
  }
