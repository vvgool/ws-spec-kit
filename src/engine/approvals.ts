...
export async function requestArtifactApproval(input: { cwd: string; workItemId: string; stageId: string; attemptId: string; artifactPath: string }): Promise<RuntimeApproval> {
  ...
  if (projection.stages[input.stageId]?.status !== "validating") throw new ApprovalError("WSPEC_APPROVAL_NOT_READY", "Stage 尚未进入 validating。");
  ...
  return mutateControlPlane({
    ...
    mutate: (current) => {
      if (current.stages[input.stageId]?.status !== "validating") throw new ApprovalError("WSPEC_APPROVAL_NOT_READY", "Stage 尚未进入 validating。");
      const next = {
        ...current,
...
export async function decideArtifactApproval(input: { cwd: string; workItemId: string; requestId: string; decision: "approve" | "reject"; terminal: { isTTY?: boolean }; reason?: string }): Promise<RuntimeApproval> {
  if (input.terminal.isTTY !== true) throw new ApprovalError("WSPEC_INTERACTIVE_TTY_REQUIRED", "批准或拒绝必须来自真实交互式 TTY。");
  let projection = await readControlPlane(input.cwd, input.workItemId);
  const request = projection.approvals[input.requestId];
  if (request?.status !== "pending") throw new ApprovalError("WSPEC_APPROVAL_NOT_PENDING", "审批请求不存在或已经处理。");
  const worktree = await worktreeFor(input.cwd, input.workItemId);
  ...
  if (currentDigest !== request.workspaceTreeDigest) {
    ...
    throw new ApprovalError("WSPEC_APPROVAL_EXPIRED", "审批绑定的工作区已经变化，请重新请求审批。");
  }
  ...
  return mutateControlPlane({
    ...
    mutate: (current) => {
      ...
      if (pending?.status !== "pending") throw new ApprovalError("WSPEC_APPROVAL_NOT_PENDING", "审批请求不存在或已经处理。");
      const next = {
        ...current,
