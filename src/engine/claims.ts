...
async function locations(cwd: string, workItemId: string): Promise<{ worktree: string; manifest: string; workflow: string; config: string }> {
  ...
  if (cache.repositoryId !== projection.repositoryId) throw new ClaimError("WSPEC_REPOSITORY_ID_MISMATCH", "仓库身份不一致。");
...
export async function claimStage(input: { cwd: string; workItemId: string; stageId: string; actor: string; allowedPaths?: string[]; now?: string }): Promise<Claim> {
  ...
  const claim = await mutateControlPlane({
    ...
    if (existing !== undefined) {
      if (new Date(existing.expiresAt) > now) throw new ClaimError("WSPEC_STAGE_ALREADY_CLAIMED", `Stage ${input.stageId} 已被领取。`);
    ...
    if (projection.stages[input.stageId]?.status !== "ready") throw new ClaimError("WSPEC_STAGE_NOT_READY", `Stage ${input.stageId} 当前不可领取。`);
...
export async function buildStageContext(claim: Claim): Promise<StageContext> {
  ...
  if (active?.attemptId !== claim.attemptId || active.claimToken !== claim.claimToken) throw new ClaimError("WSPEC_ATTEMPT_NOT_ACTIVE", "Attempt 已失效。");
  ...
  if (stage === undefined) throw new ClaimError("WSPEC_STAGE_NOT_FOUND", `找不到 Stage ${claim.stageId}。`);
  const unsigned = {
  ...
  if (content === undefined) throw new ClaimError("WSPEC_CONTEXT_INVALID", "Context 无法规范化。");
  ...
  return mutateControlPlane({
    ...
    mutate: (current) => {
      ...
      if (currentClaim?.attemptId !== claim.attemptId || currentClaim.claimToken !== claim.claimToken) throw new ClaimError("WSPEC_ATTEMPT_NOT_ACTIVE", "Attempt 已失效。");
...
export async function renewClaim(input: { cwd: string; workItemId: string; stageId: string; attemptId: string; claimToken: string; now?: string }): Promise<RuntimeClaim> {
  ...
  return mutateControlPlane({
    ...
    if (claim?.attemptId !== input.attemptId || claim.claimToken !== input.claimToken) throw new ClaimError("WSPEC_ATTEMPT_NOT_ACTIVE", "Attempt 或 Claim 令牌已经失效。");
    const now = new Date(input.now ?? Date.now());
    if (new Date(claim.expiresAt) <= now) throw new ClaimError("WSPEC_CLAIM_EXPIRED", "Claim 已过期。");
    const config = parse(await readFile(place.config, "utf8")) as { runtime: { claimTtlSeconds: number } };
