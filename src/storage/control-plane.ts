...
import { parseApplicationSnapshot, type ApplicationSnapshot } from "../application/snapshot.js";
...
import { verifySourceArtifact, type SourceArtifact, type SourceArtifactReference } from "../registry/connectors/requirement-source.js";
...
function sourceReferenceFromEvents(events: readonly StoredEvent[], workItemId: string): SourceArtifactReference {
  const captured = events.filter((event) => event.eventType === "source.captured");
  if (captured.length !== 1) {
    throw new ControlPlaneStorageError("WSSPEC_SOURCE_SNAPSHOT_CHANGED", "Work Item 缺少唯一可信 Source 捕获事件。");
  }
...
export async function resolveWorkItemContext(cwd: string, workItemId: string): Promise<ResolvedWorkItemContext> {
  ...
  if (locator.repositoryId !== repository.repositoryId || locator.workItemId !== workItemId) {
  ...
  const materialized = locator.materialized !== false;
  if (!materialized && locator.authorityRoot !== "authority") {
    throw new ControlPlaneStorageError("WSSPEC_WORK_ITEM_LOCATION_INVALID", "未物化 Work Item locator 必须指向 common-dir authority。");
  }
  if (materialized && (typeof locator.worktree !== "string" || path.isAbsolute(locator.worktree) || locator.worktree.split(/[\\/]/u).includes(".."))) {
  ...
  try {
    ...
    worktree = materialized ? await realpath(path.resolve(cachedRoot, locator.worktree!)) : cachedRoot;
  } catch {
  ...
  if (materialized && (worktreeRelative === "" || worktreeRelative.startsWith("..") || path.isAbsolute(worktreeRelative))) {
...
export async function resolveWorkItemContextFromControlPlane(controlPlane: string, workItemId: string): Promise<ResolvedWorkItemContext> {
  ...
  const cachedRoot = await realpath(cache.repositoryRoot);
  const materialized = locator.materialized !== false;
  if (!materialized && locator.authorityRoot !== "authority") {
    throw new ControlPlaneStorageError("WSSPEC_WORK_ITEM_LOCATION_INVALID", "未物化 Work Item locator 必须指向 common-dir authority。");
  }
...
async function resolveControlPlane(cwd: string, workItemId: string): Promise<ResolvedWorkItemContext> {
  return resolveWorkItemContext(cwd, workItemId);
}
...
export async function readCapturedSourceReference(cwd: string, workItemId: string): Promise<SourceArtifactReference> {
  const resolved = await resolveControlPlane(cwd, workItemId);
  return readCapturedSourceReferenceFromControlPlane(resolved.directory, workItemId);
}

export async function readCapturedSourceReferenceFromControlPlane(controlPlane: string, workItemId: string): Promise<SourceArtifactReference> {
  const events = await readEvents(controlPlane).catch((error: unknown) => {
    if (error instanceof EventStoreError) throw new ControlPlaneStorageError(error.code, error.message);
    throw error;
  });
  return sourceReferenceFromEvents(events, workItemId);
}

export interface AuthenticatedApplicationSourceAuthority {
  ...
  sourceReference: SourceArtifactReference;
}

export async function authenticateApplicationSourceAuthority(input: {
  controlPlane: string;
  worktree: string;
  authorityRoot?: string;
  workItemId: string;
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "ENOENT") {
      throw new ControlPlaneStorageError("WSSPEC_SOURCE_SNAPSHOT_CHANGED", "Work Item 缺少可信 Application 快照。");
    }
    throw error;
  }

  const manifestText = await readFile(path.join(itemRoot, "work-item.yaml"), "utf8");
  const anchor = await readApplicationAnchorFile(input.controlPlane);
  if (anchor === undefined) {
    throw new ControlPlaneStorageError("WSSPEC_SOURCE_SNAPSHOT_CHANGED", "Work Item 缺少可信 Application 锚点。");
  }
  ...
  const manifest = parse(manifestText) as WorkItem;
  ...
  try {
    application = parseApplicationSnapshot(JSON.parse(applicationText));
  } catch {
    throw new ControlPlaneStorageError("WSSPEC_APPLICATION_SNAPSHOT_INVALID", "Application 快照不是合法的严格快照。");
  }
  if (manifest.workItemId !== input.workItemId
    || (input.repositoryId !== undefined && manifest.repositoryId !== input.repositoryId)) {
    throw new ControlPlaneStorageError("WSSPEC_REPOSITORY_ID_MISMATCH", "Work Item manifest 与控制面身份不一致。");
  }

  const events = await readEvents(input.controlPlane).catch((error: unknown) => {
    if (error instanceof EventStoreError) throw new ControlPlaneStorageError(error.code, error.message);
    throw error;
  });
  const eventSourceReference = sourceReferenceFromEvents(events, input.workItemId);
  ...
  if (!sameCanonicalValue(eventSourceReference, manifestSourceReference)
    || !sameCanonicalValue(eventSourceReference, application.source)) {
  ...
  return {
    ...
    manifest,
    source,
    sourceReference: eventSourceReference,
  };
}
