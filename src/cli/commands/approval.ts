...
export async function runApprovalCommand(args: string[], decision: "approve" | "reject", terminal = process.stdin): Promise<void> {
  if (terminal.isTTY !== true || args.includes("--yes")) throw new ApprovalError("WSPEC_INTERACTIVE_TTY_REQUIRED", "审批必须在真实 TTY 中现场确认，且不支持 --yes。");
  const [workItemId, requestId] = args;
  if (workItemId === undefined || requestId === undefined) throw new ApprovalError("WSPEC_ARGUMENT_REQUIRED", "缺少 Work Item ID 或审批请求 ID。");
  const projection = await readControlPlane(process.cwd(), workItemId);
  const request = projection.approvals[requestId];
  if (request === undefined) throw new ApprovalError("WSPEC_APPROVAL_NOT_PENDING", "审批请求不存在。");
  ...
  try {
    ...
    if (answer.trim() !== decision) throw new ApprovalError("WSPEC_APPROVAL_CONFIRMATION_MISMATCH", "现场确认内容不匹配。");
  } finally {
    readline.close();
  }
