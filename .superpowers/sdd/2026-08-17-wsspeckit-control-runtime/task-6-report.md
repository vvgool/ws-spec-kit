...
| Governed | 独立 Reviewer 冲突抛异常，导致上一 submit 事务回滚，无法通过协议换 Actor 继续。 | 原实现者得到 retryable typed `blocked`；独立 Reviewer 完成 Review；complete audit 与本地外部回执满足 Close。 | Close 后损坏 projection，再由公开 `inspect` 从事件恢复 closed；独立 Reviewer handoff 不消耗或重置循环状态。 |
...
- workspace digest 排除 `.wsspec/work-items/**` 与 `.wsspec/archive/**`。
...
- WorkPackage gate schema 与 Evidence gate schema 统一允许 `docs.integrity`；文档完整性 Gate 使用与提交边界一致的 glob 语义。
