...
async function prepare() {
  ...
  const workItemId = "WSK-ARCHIVE"; const item = await createWorkItem({ root, workItemId, title: "归档", source: { type: "prompt", content: "归档" } }); const worktree = path.join(root, item.execution.worktree);
...
test("close rejects an unverified Work Item", async () => {
  const fixture = await prepare(); await assert.rejects(closeWorkItem({ cwd: fixture.root, workItemId: fixture.workItemId }), (error: unknown) => error instanceof ArchiveError && error.code === "WSPEC_WORK_ITEM_NOT_VERIFIED");
});
...
test("close exports an audit snapshot and makes the control plane read-only", async () => {
  ...
  await assert.rejects(transitionRuntime({ cwd: fixture.root, workItemId: fixture.workItemId, scope: "work-item", to: "cancelled", idempotencyKey: "late" }), (error: unknown) => error instanceof Error && "code" in error && (error as Error & { code: string }).code === "WSPEC_CONTROL_PLANE_READ_ONLY");
  ...
  await assert.rejects(transitionRuntime({ cwd: fixture.root, workItemId: fixture.workItemId, scope: "work-item", to: "cancelled", idempotencyKey: "later" }), (error: unknown) => error instanceof Error && "code" in error && (error as Error & { code: string }).code === "WSPEC_CONTROL_PLANE_READ_ONLY");
});
...
test("recover rebuilds the archive after a crash following the close event", async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) => error instanceof ArchiveError && error.code === "WSPEC_ARCHIVE_WRITE_FAILED",
  );
...
test("close rejects incomplete stages and pending approvals", async () => {
  ...
  await assert.rejects(closeWorkItem({ cwd: incomplete.root, workItemId: incomplete.workItemId }), (error: unknown) => error instanceof ArchiveError && error.code === "WSPEC_REQUIRED_STAGE_INCOMPLETE");
  ...
  await assert.rejects(closeWorkItem({ cwd: pending.root, workItemId: pending.workItemId }), (error: unknown) => error instanceof ArchiveError && error.code === "WSPEC_APPROVAL_PENDING");
});
