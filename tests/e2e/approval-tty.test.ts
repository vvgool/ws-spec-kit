...
async function approvalFixture() {
  ...
  await mkdir(path.join(worktree, "artifacts")); await writeFile(path.join(worktree, artifactPath), `---\nartifactType: specification\nschemaVersion: 1\nworkItemId: ${workItemId}\nstageId: define\nattemptId: attempt-tty\nrevision: 1\ncontentHash: ${contentHash}\n---\n${body}`); const request = await requestArtifactApproval({ cwd: root, workItemId, stageId: "define", attemptId: "attempt-tty", artifactPath, artifactType: "specification" }); return { root, worktree, workItemId, requestId: request.requestId };
}
